...
<div class="w ofc case">
	...
	<div class="box bg_white pb">
		...
		<c:forEach items="${buyer.cart_infos }" var="cartInfo">
			<tr>
				<td>${cartInfo.product.no }</td>
				<td class="img48x20">
					<span class="inb"><img src="http://localhost:8088/pic-web/${cartInfo.imgUrl}"></span>
					<a target="block" href="javascript:void(0);">${cartInfo.product.name }--${cartInfo.colorName }--${cartInfo.sku.size }</a>
				</td>
				<td>￥${cartInfo.sku.skuPrice }元</td>
				<td>x${cartInfo.quantity }</td>
				<td>${cartInfo.sku.stockInventory }</td>
			</tr>
			</c:forEach>
		</tbody>
		</table>
	 ...
	 <div class="confirm mt">
		...
		<div class="ofc">
			<div class="r">
				<dl class="total">
					<dt>订单金额：${buyer.totalPrice+buyer.totalFee }<cite>(共<var id="totalNum">${buyer.cart.total_quantity }</var>个商品)</cite></dt>
					<dd><em class="l">商品金额：</em>￥<var>${buyer.totalPrice }</var></dd>
					<dd><em class="l">返现：</em>￥<var>0.00</var></dd>
					<dd><em class="l">运费：</em>￥<var>${buyer.totalFee }</var></dd>
					<dd class="orange"><em class="l">应付总额：</em>￥<var id="totalMoney">${buyer.totalPrice+buyer.totalFee }</var></dd>
